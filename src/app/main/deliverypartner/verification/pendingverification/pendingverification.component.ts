...
export class PendingverificationComponent implements OnInit {
    ...
    this.contentHeader = {
      ...
      breadcrumb: {
        ...
        links: [
          ...
          {
            name: 'Delivery Partner Verification',
            isLink: false
          }
        ]
      }
  ...
  // modal Open Success
  modalOpenSuccess(modalSuccess, partnerId: any) {
    this.spinnerService.show();
  ...
  verifyAllPartnerDetails(partnerId: any) {
    this.spinnerService.show();
    this.delivery.verifyAllPartnerDetails(partnerId).subscribe((res) => {
      this.spinnerService.hide();
      if (res["status"] == true) {
        this.message = res["message"];
